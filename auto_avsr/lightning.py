...
class ModelModule(LightningModule):
    ...
    def on_test_epoch_end(self):
        ...
        if tl and tl > 0:
            self.log("wer", self.total_edit_distance / tl)
        else:
            # No references provided → just log 0 and continue
            self.log("wer", 0.0)

        # do the same for CER if present:
        if hasattr(self, "total_char_length"):
            tcl = getattr(self, "total_char_length", 0)
            if tcl and tcl > 0:
                self.log("cer", self.total_char_edit_distance / tcl)
            else:
                self.log("cer", 0.0)

    # add anywhere in the class (e.g., after test_step)
    def on_test_batch_end(self, outputs, batch, batch_idx, dataloader_idx=0):
        try:
            pred = None
            if isinstance(outputs, dict):
                # common keys used in AVSR repos
                for k in ("pred_text", "pred", "hyp", "hyp_text"):
                    if k in outputs:
                        pred = outputs[k]
                        break
                # sometimes list/tuple per-batch
                if pred is None and "pred_texts" in outputs:
                    pred = outputs["pred_texts"]
            # print nicely
            if pred is not None:
                if isinstance(pred, (list, tuple)):
                    for s in pred:
                        if isinstance(s, (list, tuple)): s = " ".join(map(str, s))
                        print(f"[DECODE] {s}")
                else:
                    if isinstance(pred, (list, tuple)): pred = " ".join(map(str, pred))
                    print(f"[DECODE] {pred}")
        except Exception as e:
            print("[DECODE] (could not print prediction)", e)
